\section{Temporal Bitcoin is Timely}
...
\begin{proof}
  ...
  % TODO Note: Maybe this need to be -k-1
  The transaction $\tx$ is contained in some block $B$ of $\chain_2[{|\chain_1[{:}{-k-1}]|}{:}]$.
  % TODO Note: Maybe this need to be -k, else the transaction could be in C1 as well
  ...
  % TODO Note: $r \geq r^*$ since recorded rounds are not decreasing
  therefore $r^* < r_1 - v - 1$.
  % TODO Note: $r^* \geq r_1 - v + 1$ since recorded rounds are not decreasing
  ...
  \[
     ...
     k + \ell\,.
  \]
  %TODO This could be < k + \ell if we closed the bounds
  ...
  %TODO at most $k + \ell$ blocks apart
  of $B^*$.
