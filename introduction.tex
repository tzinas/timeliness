...
\begin{figure}
  ...
  \begin{tikzpicture}[
    block/.style={draw, thick, align=center, minimum width=3cm, minimum height=1cm},
    ...
    % \node[block, minimum width=3.6cm, minimum height=0.5cm, right=1cm of bitcoin.north east, anchor=north west] (abcv) {Abstract chain virtues};
    % \node[block, minimum width=1.18cm, minimum height=0.5cm, below=0cm of abcv.south west, anchor=north west] (cq) {CQ};
    % \node[block, minimum width=1.18cm, minimum height=0.5cm, right=0cm of cq.east, anchor=west] (cg) {CG};
    % \node[block, minimum width=1.18cm, minimum height=0.5cm, right=0cm of cg.east, anchor=west] (cp) {CP};
    \node[block, lightblue, minimum width=3.6cm, right=1cm of bitcoin, align=center] (abcv) {Abstract chain virtues\\CQ \quad CG \quad CP};
    %
    % \def\abcvWidth{3.6cm}
    % % abcv node without border
    % \node[minimum width=\abcvWidth, minimum height=0.5cm, right=1cm of bitcoin.north east, anchor=north west, inner sep=0pt] (abcv) {};

    % % Text for abcv
    % \node[anchor=north] at (abcv.north) {Abstract chain virtues};

    % % Nodes inside abcv with calculated widths
    % \node[block, minimum width=\abcvWidth/3, minimum height=0.5cm, below=0cm of abcv.south west, anchor=north west] (cq) {CQ};
    % \node[block, minimum width=\abcvWidth/3, minimum height=0.5cm, right=0cm of cq.east, anchor=west] (cg) {CG};
    % \node[block, minimum width=\abcvWidth/3, minimum height=0.5cm, right=0cm of cg.east, anchor=west] (cp) {CP};
    ...
    \node[block, lightblue, right=1cm of streamlet, anchor=west, minimum width=2cm] (recency) {Recency};
    \node[block, lightblue, below=of recency.south west, anchor=west, minimum width=2cm] (freshness) {Freshness};
    \node[block, lightblue, below=2.5cm of abcv.south east, anchor=east, minimum width=1cm] (safety) {Safety};
    ...
    %   % Arrows
    \draw[arrow] (bitcoin.east) -- (abcv.west) node[pos=0.5, below, font=\footnotesize] {\cite{backbone}};

    \draw[arrow] (ouroboros.west) -- (abcv.east) node[pos=0.5, below, font=\footnotesize] {\cite{ouroboros}};

    \draw[decorate, decoration={brace, amplitude=5pt, mirror}, thin] (abcv.south west) -- ++(2.4cm,0);
    % Define midpoint of the curly brace
    \coordinate (braceMid) at ($(abcv.south west)!0.33!(abcv.south east)$);
    ...
    \coordinate (abcvcp) at ($(abcv.south west)!0.83!(abcv.south east)$);
    ...
    \draw[arrow] ($(timeliness.north west)!0.67!(timeliness.south west)$) -- ($(supersafety.north east)!0.67!(supersafety.south east)$) node[pos=0.5, below, font=\footnotesize, align=center] {Thm~\ref{thm:backward-reduction}\\($\nu=0$)};
